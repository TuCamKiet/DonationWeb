...
export const api = {
  ...
  // Products
  products: (params?: { category?: string; featured?: boolean; q?: string; sort?: string }) =>
    request<Product[]>('/products', { query: params }),
  product: (slug: string) => request<Product>(`/products/${encodeURIComponent(slug)}`),

  // Stories
  stories: (kind?: 'artisan' | 'school') => request<Story[]>('/stories', { query: { kind } }),
  story: (slug: string) => request<Story>(`/stories/${encodeURIComponent(slug)}`),
